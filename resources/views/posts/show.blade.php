            ...
            @auth
                @if ($post->user_id === auth()->user()->id)
                    <form action="{{route('posts.destroy', $post)}}" method="post">
                        @method('DELETE')
                        @csrf
                        <input type="submit" value="Eliminar Publicacion" class="bg-red-500 hover:bg-red-600 p-2 rounded text-white font-bold mt-2 cursor-pointer">
                    </form>
                @endif
            @endauth
        </div>
                ...
                <div class="shadow bg-white p-5 mb-5">
                    ...
                    <form action="{{route('comentarios.store', ['post' =>$post, 'user'=>$user])}}" method="POST">
                        ...
                        <div class="mb-5">
                            ...
                            <textarea id="comentario" name="comentario" placeholder="Agregue un comentario" class="border p-3 w-full rounded-lg @error('comentario')
            ...
            <div class="bg-white shadow mb-5 max-h-96 overflow-y-scroll mt-10">
                @if ($post->comentarios->count())
                    @foreach ($post->comentarios as $comentario)
                        <div class="p-5 border-gray-380 border-b">
